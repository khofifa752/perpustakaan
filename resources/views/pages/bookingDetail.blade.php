...
.info-row { display: flex; gap: 1rem; font-size: .875rem; padding: .4rem 0; border-bottom: 1px solid #f5f0e8; }
...
.alert-terlambat { background: #fef2f2; border: 1px solid #fecaca; color: #dc2626; border-radius: 12px; padding: 11px 16px; font-size: .875rem; font-weight: 600; }
...
<div class="detail-wrap">
  ...
  @if($terlambat)
    <div class="alert-terlambat" style="margin-bottom:1rem;">
      ⚠️ Buku ini sudah terlambat {{ $hariTerlambat }} hari! Segera ajukan pengembalian.
    </div>
  @endif
      ...
      <div class="info-card">
        ...
        <div class="info-row">
          ...
          <span class="info-value">
            ...
            @if($terlambat)
              <span style="font-size:.72rem; font-weight:700; color:#dc2626; margin-left:6px;">⚠️ Terlambat {{ $hariTerlambat }} hari</span>
            @endif
          </span>
        </div>
        <div class="info-row">
        ...
        <div class="info-row">
          <span class="info-label">Tenggat Kembali</span>
          <span class="info-value" style="{{ $terlambat ? 'color:#dc2626; font-weight:600;' : '' }}">
            {{ \Carbon\Carbon::parse($booking->expired_at)->translatedFormat('d F Y') }}, {{ $booking->created_at->setTimezone('Asia/Jakarta')->format('H:i') }} WIB
          </span>
        </div>
